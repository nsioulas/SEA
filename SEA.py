...
def SEA(df, condit_variables, threshold_values, conditions, time_around, which_one, mean_or_median, resample_rate):
   ...
   -  resample_rate (str): The rate to resample the extracted data to.
    ...
    # Extract specified variable for each event
    keep_vals             = [list(df[which_one].iloc[start:end].values) for start, end in zip(start_indices, end_indices)]
    
    if len(keep_vals)>0:
        # Calculate the mean or median of the extracted data
        if mean_or_median:
